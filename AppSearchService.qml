...
import Quickshell
import qs.Common
import "../Common/PinyinHelper.js" as PinyinHelper
...
Singleton {
    ...
    function searchApplications(query) {
        ...
        for (const app of visibleApps) {
            ...
            if (matchType === "none" && id && id.includes(queryLower)) {
                textScore = 350;
                matchType = "id";
            }
            ...
            if (matchType === "none") {
                const genericPinyin = genericName ? PinyinHelper.toPinyinCompact(genericName) : "";
                if (genericPinyin && genericPinyin.includes(queryLower)) {
                    textScore = 250;
                    matchType = "pinyin_generic";
                }
            }
